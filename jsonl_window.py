...
def read_jsonl_recent(path, days=60, now=None, ts_field="ts", stop_after_old=25000):
    ...
    (実測: raw_comments.jsonlで最大71行連続・平常時)が散発する。これを境界より古い行
    1行だけで打ち切ると直近データを不当に切り捨ててしまうため、境界より古い行が
    stop_after_old行"連続"した時点で初めて打ち切る(単発の逆行行はスキップして
    読み進める=結果には含めない)。

    ★2026-09-06引き上げ(2026-08-21 sentiment_24h_bridge.py重大障害の再発防止)。
    旧既定値500は「平常時の逆行ブロックは最大71行」という前提の約7倍マージン
    だったが、2026-08-21にcollect側の排他ロック欠如(既に修正済み)により
    逆行/重複ブロックが1クラスタで実測20,104行まで膨張し、旧既定値を大幅に
    超過して直近ウィンドウの読み込みが誤って早期打ち切りされ、投稿量が
    軒並み0になる実害を起こした(bridgeは即Disable・手動復旧・原因究明済み)。
    collect_lock導入により新規の同規模ブロック発生は防止されているはずだが、
    「平常時の実測値の数倍」という設計自体が異常系(排他制御の欠陥等)の再発に
    弱いと判明したため、既定値を実測最大の約1.2倍(20,104→25,000)へ引き上げ、
    同規模の異常が再発してもこの関数単体では即座に壊れない安全マージンを持たせた
    (それでも全件読込にはならない=当時のraw_comments.jsonlは数十万行規模)。
